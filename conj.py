...
def laplacian_pieces(G, nodelist=None, weight='weight'):
    ...
    if nodelist is None: nodelist = G.nodes()
...
def ando_lin(eigen_values):
    ...
    try:
        ...
        raise Exception(f'Division by zero in Ando-Lin bound construction: negative = {sn}, positive = {sp}')
...
def compare(G,name,tmp_file,verbose=False):
    ...
    if verbose: print(f'Testing {name}')
...
def named_graphs(min_order, max_order, dotdir, verbose=False):
    ...
    for filename in files:
        ...
        if G.size() > 0 and len(G) >= min_order and len(G) <= max_order and nx.is_connected(G):
